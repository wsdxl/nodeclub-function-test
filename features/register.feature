...
Feature: register function test
    ...
    Scenario: 输入正确的注册信息，成功登录
        When 导航到注册页面
        Then 用户名输入'123457',密码输入'123',确认密码输入'123',邮箱输入'123456@qq.com'
        Then 点击提交按钮，注册成功，提示,'欢迎加入 Nodeclub！我们已给您的注册邮箱发送了一封邮件，请点击里面的链接来激活您的帐号。'
    @register2
    Scenario Outline: 注册失败，得到提示信息
        Given 导航到注册页面
        When 用户名输入'<username>',密码输入'<pass>',确认密码输入'<re_pass>',邮箱输入'<email>'
        Then 点击注册的按钮，得到提示'<message>'
        Examples:
